...
var SendForgotPasswordEmailService = /** @class */ (function () {
    function SendForgotPasswordEmailService(userRepository, userTokenRepository, mailProvider) {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        user = _b.sent();
                        if (!user) {
                            throw new AppError_1.default('User do not exits');
                        }
                        return [4 /*yield*/, this.userTokenRepository.generate(user.id)];
                    case 2:
                        token = (_b.sent()).token;
                        forgotPasswordTemplate = path_1.default.resolve(__dirname, '..', 'views', 'forgot_password.hbs');
                        return [4 /*yield*/, this.mailProvider.sendEmail({
                                to: {
                                    name: user.name,
                                    email: user.email,
                                },
